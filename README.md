...
数据集来自 Kaggle：Brazilian E-Commerce Public Dataset by Olist。包含约 10 万条订单记录，涵盖客户、订单、支付、评价、商品、卖家等维度。

🛠️ 分析流程

1. 数据清洗（01_数据清洗.ipynb）

加载原始表（customers、orders、order_items、order_payments、order_reviews、products、sellers、geolocation、product_category_translation）。
探查各表基本信息、缺失值、异常值。
缺失值处理：
...
品类销售额（含/不含运费）、销量、平均评分、各品类销售额最高的城市。
保存特征表至 features/（或中文文件夹）。
3. 可视化分析与业务洞察（03_可视化分析.ipynb）
...
├── cleaned_data/               # 清洗后数据（中间文件）
├── features/                    # 特征表（用户、订单、产品维度）
├── 01_数据清洗.ipynb
├── 02_特征工程.ipynb
├── 03_可视化分析.ipynb
├── README.md
